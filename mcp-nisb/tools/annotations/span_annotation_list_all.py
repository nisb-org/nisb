# /opt/mcp-gateway/mcp-nisb/tools/annotations/span_annotation_list_all.py
import json
import os
import re
...
def nisb_annotation_list_all(args: Dict[str, Any]) -> Dict[str, Any]:
    """
    跨库列出 Span 批注（支持：可选 library_id 过滤 + 时间范围 + limit）。
    读取：{user_base}/annotations/library_doc_span_annotations.jsonl
    返回：{status:'success', annotations:[...], total:int}
    """
    ...
    path = os.path.join(user_base, "annotations", "library_doc_span_annotations.jsonl")
    ...
    for it in rows:
        ...
        if t == "library_doc_span_annotation_tombstone" or bool(it.get("tombstone")):
            deleted.add(aid)
            state.pop(aid, None)
            continue
    ...
    return {"status": "success", "annotations": out[:limit], "total": len(out)}
