...
def nisb_doc_convert_to_note(
    ...
    """
    Convert a .doc file to .docx with LibreOffice, then reuse the DOCX-to-note converter.
    """
    user_lock_path = ""
    target_lock_path = ""
    ...
    try:
        ...
        doc_rel = _strip_invisible(_safe_rel_path(doc_path))
        if "?" in doc_rel:
            doc_rel = doc_rel.split("?", 1)[0]
        if "#" in doc_rel:
            doc_rel = doc_rel.split("#", 1)[0]
        doc_rel = _strip_invisible(doc_rel)

        ext = os.path.splitext(doc_rel)[1].lower()
        if ext != ".doc":
            return {
                "success": False,
                "message": "doc_path must end with .doc",
                "debug_doc_path_repr": repr(doc_rel),
                "debug_ext": ext,
            }

        abs_doc = _resolve_under_user(uid, doc_rel)
        if not os.path.exists(abs_doc):
            return {"success": False, "message": f"DOC not found: {doc_rel}"}
        ...
        in_dir_rel = os.path.dirname(doc_rel).replace("\\", "/").strip("/")
        stem = os.path.basename(doc_rel).rsplit(".", 1)[0]
        docx_rel = f"{in_dir_rel}/{stem}.docx" if in_dir_rel else f"{stem}.docx"
        ...
        target_lock_path = abs_docx + ".office_doc_to_docx.lock"
        try:
        ...
        docx_manifest_abs = abs_docx + ".office_doc_manifest.json"

        def _docx_manifest_matches(m: dict) -> bool:
            try:
                if not isinstance(m, dict):
                    return False
                if str(m.get("tool") or "") != "nisb_doc_convert_to_note":
                    return False
                if str(m.get("source_path") or "") != doc_rel:
                    return False
                if int(m.get("source_size") or 0) != int(src_size or 0):
                    return False
                if int(m.get("source_mtime") or 0) != int(src_mtime or 0):
                    return False
                if str(m.get("docx_path") or "") != docx_rel:
                    return False
                return True
            except Exception:
                return False

        if os.path.exists(abs_docx) and (not overwrite):
            m0 = _safe_read_json(docx_manifest_abs)
            if not _docx_manifest_matches(m0):
                return {
                    "success": False,
                    "message": f"Target DOCX exists (overwrite=false): {docx_rel}",
                    "docx_path": docx_rel,
                }

        if overwrite and os.path.exists(abs_docx):
            m0 = _safe_read_json(docx_manifest_abs)
            if os.path.exists(abs_docx) and (not _docx_manifest_matches(m0)):
                return {
                    "success": False,
                    "message": "Refuse to overwrite: docx manifest missing or not match doc_path.",
                    "docx_path": docx_rel,
                }

        out_dir_abs = os.path.dirname(abs_docx) or _user_root(uid)
        ...
        if (not run_res.get("ok")) or (not os.path.exists(abs_docx)):
            return {
                "success": False,
                "message": "DOC -> DOCX conversion failed (soffice).",
                "uid": uid,
                "doc_path": doc_rel,
                "docx_path": docx_rel,
                "cmd": run_res.get("cmd"),
        ...
        _atomic_write_json(docx_manifest_abs, {
            "tool": "nisb_doc_convert_to_note",
            "uid": uid,
            "source_path": doc_rel,
        ...
        from tools.office.docx_convert_to_note import nisb_docx_convert_to_note

        res = nisb_docx_convert_to_note({
            "docx_path": docx_rel,
            "uid": uid,
        ...
        if (not keep_intermediate) and res and res.get("success"):
            try:
                ...
                pass
            try:
                if os.path.exists(docx_manifest_abs):
                    os.remove(docx_manifest_abs)
            except Exception:
                pass

        if isinstance(res, dict):
            res["source_doc_path"] = doc_rel
            res["intermediate_docx_path"] = docx_rel
            res["pipeline"] = "doc->docx->md"
        return res
        ...
        return {"success": False, "message": f"DOC convert failed: {str(e)}"}
    finally:
