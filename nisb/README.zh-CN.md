...
NISB 把主要的 workspace 状态保存在独立于应用本身的持久化数据目录下。

全新部署可以通过一套标准命令完成。笔记、文件、知识库、证据库、Room notebook 和 workspace 配置等主要用户状态都保存在 `/opt/nisb-data` 下。

迁移或恢复一个 NISB 实例，主要动作是重新部署应用并移动数据目录。不需要手工重建散落在多个服务或外部依赖里的配置，就能还原一个可用的 workspace。

这对自托管 AI 工作区很重要。Workspace 状态不应该被锁死在一个难以复现、难以备份、难以迁移的脆弱一次性环境里。
